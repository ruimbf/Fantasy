...
{
    private readonly string _imageContainerName = "teams";
    ...
    public async Task<ActionResponse<Team>> AddAsync(TeamDTO teamDTO)
    {
        ...
        {
            var image = Convert.FromBase64String(teamDTO.Image!);
            team.Image = await _fileStorage.SaveFileAsync(image, _imageExtension, _imageContainerName);
        }
    ...
    public async Task<ActionResponse<Team>> UpdateAsync(TeamDTO teamDTO)
    {
        ...
        {
            var image = Convert.FromBase64String(teamDTO.Image!);
            currentTeam.Image = await _fileStorage.SaveFileAsync(image, _imageExtension, _imageContainerName);
        }
